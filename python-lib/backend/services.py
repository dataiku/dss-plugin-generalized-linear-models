...
class MockDataService:
    ...
    def deploy_model(self, request_json: dict):
        logger.info("Local set up: No model deployment completed")
        time.sleep(2)
        return {'message': 'Model deployed successfully.'}
    
    def delete_model(self, request_json: dict):
        logger.info("Local set up: No model deletion completed")
        time.sleep(1)
    ...
    def get_latest_mltask_params(self, request_json: dict):
        ...
        logger.info(f"Returning Params {setup_params}")
        return setup_params
    ...
    def get_dataset_columns(self, request_json: dict):
        ...
        logger.info(f"Successfully retrieved column for dataset '{dataset_name}': {[col['column'] for col in cols_json]}")
...
class DataikuDataService:
    ...
    def train_model(self, request_json: dict):
        logger.info(f"Initalising Model Training with request {request_json}")
        
        self.visual_ml_config = DKUVisualMLConfig()
        self.visual_ml_config.update_model_parameters(request_json)

        logger.debug("Creating Visual ML Trainer")
        ...
        logger.debug(f"Model error message is {error_message}")
        logger.debug(f"Model details are {model_details}")
        
        if not error_message:
            logger.info("Model trained and cache updated")
            return {'message': 'Model training completed successfully.'}
        else:
            logger.debug(f"Model training error: {error_message}")
            raise ValueError(f"Model training error: {error_message}")
    
    def deploy_model(self, request_json: dict):
        logger.info(f"Initalising Model Deployment with request {request_json}")
    ...
    def delete_model(self, request_json: dict):
        logger.info(f"Deleting Model with request {request_json}")
    ...
    def get_latest_mltask_params(self, request_json: dict):
        ...
        logger.info(f"Recieved request for latest params for: {full_model_id}")
        
        model_retriver = VisualMLModelRetriver(full_model_id)
        setup_params = model_retriver.get_setup_params()
    
        logger.info(f"Returning setup params {setup_params}")
        return setup_params
    ...
    def get_models(self, request_json: dict):
        ...
        logger.info(f"Mltask has : {len(self.visual_ml_trainer.mltask.get_trained_models_ids())} Models")
        
        models = format_models(ml_task)
        logger.info(f"models from global ML task is {models}")
        return models
    ...
    def get_predicted_base(self, request_json: dict):
        ...
        logger.info(f"Successfully generated predictions. Sample is {predicted_base_variable}")
        
        return predicted_base_variable.to_dict('records')
    
    def get_base_values(self, request_json: dict):
        full_model_id = request_json["id"]
        logger.info(f"Request received for base_values for {full_model_id}")
    ...
    def get_lift_data(self, request_json: dict):
        ...
        logger.info(f"Successfully generated Lift chart data")
    ...
    def get_relativities(self, request_json: dict):
        ...
        logger.info(f"relativites are {relativities_df.head()}")
        return relativities_df.to_dict('records')
    
    def get_variable_level_stats(self, request_json: dict):
        logger.info("Getting Variable Level Stats")
        full_model_id = request_json["id"]
        logger.info(f"for Model ID: {full_model_id}")
        ...
        logger.info(variable_stats)
        logger.info(variable_stats.columns)
    ...
    def export_model(self, request_json: dict):
        try:
            ...
            if not model:
                logger.error("error: Model ID not provided")
            ...
            if not relativities_dict:
                logger.error(f"error: Model Cache not found for {model} cache only has {model_cache.keys()}")
            
            logger.info(f"Relativities dict for model {model} is {relativities_dict}.")
    ...
    def export_one_way(self, request_json: dict):
        ...
        try:
            ...
            logger.info(f"Model ID received: {full_model_id}")
            logger.info(f"Variable received: {variable}")
            logger.info(f"Train/Test received: {dataset}")
            logger.info(f"Rescale received: {rescale}")
    ...
    def get_dataset_columns(self, request_json):
        try:
            ...
            logger.error(f"Error retrieving target and exposure {e}")
